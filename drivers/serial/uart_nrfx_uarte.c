...
/* Determine if any enabled instance references a clock producer. */
...
#define UARTE_ANY_CLK_NODE 1
#endif

/* Clock producer management is compiled in only when at least one enabled instance
 * references a producer and the user has selected a scheme in which it is acted upon.
 */
#if defined(UARTE_ANY_CLK_NODE) && !defined(CONFIG_UART_NRFX_UARTE_CLOCK_MGMT_NONE)
